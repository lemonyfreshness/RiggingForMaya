...
def orientJointForUnreal(*args):
    ...
    cmds.joint(jointForOrient, exists = True, orientJoint = 'xzy', secondaryAxisOrient = 'xdown',  autoOrientSecondaryAxis = True,  zeroScaleOrient = True) #children = reorientChildrenValue,
...
def renameFingerJoints(*args):
    ...
    print('renameFingerJoints(), the jointChildren are: ' + str(jointChildren))
    ...
    if hasMetacarpalsValue == True:
         ...
         cmds.rename(metacarpal, fingerChain + '_metacarpal')
    ...
    for eachChild in jointChildren:
        ...
        if RLSideAbbreviation != None:
            name = name + '_' + RLSideAbbreviation
        if i == jointChildrenLength and hasEndJointValue == True:
            name = name + '_end'
...
def setupJointForReorient():
	...
	if selection != None and len(selection) == 1:
		...
		print('setupJointForReorient, Select ONE JOINT you want to reorient')
...
def cleanupReorientedJoint():
	...
	if selection != None and len(selection) == 1:
		...
		if '_ReorientJoint' in selection:
			jointForFreezeTransforms = selection
			cmds.makeIdentity(jointForFreezeTransforms, apply = True, translate = True, rotate = True, scale = True)
			originalJoint = cmds.listRelatives(selection, children = True)
			originalJoint = originalJoint[0]
			...
			cmds.delete(jointForFreezeTransforms, originalJoint + '_TempNull')
		else:
			print('cleanupReorientedJoint(), Select the _ReorientJoint')
					
	print('cleanupReorientedJoint(), Select ONE JOINT you want to reorient')
